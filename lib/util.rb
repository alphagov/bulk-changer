...
# taken from https://docs.publishing.service.gov.uk/repos.html#repos-by-type because list of gems isn't available in repos.json
def gems
  [
...
def semver_increment(version, change_type)
...
def insert_unreleased_changes(existing_content, changes)
  unreleased_header_regex = /(?:^|\n)(##? Unreleased\s*\n)/
  first_version_header_regex = /(?:^|\n)##?\s*\d+\.\d+\.\d+\s*\n/

  if existing_content =~ unreleased_header_regex
    existing_content.sub(unreleased_header_regex) do |match|
      preceding_section = match.start_with?("\n") ? "\n" : ""
      header = match[/##? Unreleased/]
      "#{preceding_section}#{header}\n\n* #{changes}\n#{match.strip.sub(/^#+ Unreleased/, '').sub(/^(\s*[-*]\s*.+\n)/, "\\1\n")}"
    end
  elsif existing_content =~ first_version_header_regex
    existing_content.sub(first_version_header_regex) do |match|
      preceding_section = match.start_with?("\n") ? "\n" : ""
      header_prefix = match.match(/(#+)\s*\d+/)[1] || "#"
      "#{preceding_section}#{header_prefix} Unreleased\n\n* #{changes}\n\n#{match.lstrip}"
    end
  else
    "# Unreleased\n\n* #{changes}\n\n" + existing_content
  end
...
def insert_new_version_section(existing_content, new_version_header, changes)
  first_version_header_regex = /(?:^|\n)((#+)\s*\d+\.\d+\.\d+\s*\n)/

  if existing_content =~ first_version_header_regex
    existing_content.sub(first_version_header_regex) do |match|
      preceding_section = match.start_with?("\n") ? "\n" : ""
      header_prefix = match.match(/(#+)\s*\d+/)[1] || "#"
      "#{preceding_section}#{header_prefix} #{new_version_header}\n\n* #{changes}\n\n#{match.lstrip}"
    end
  else
    "# #{new_version_header}\n\n* #{changes}\n\n" + existing_content
  end
